...
if 'ACCESS_TOKEN' not in os.environ or 'USER_NAME' not in os.environ:
    raise SystemExit('Missing ACCESS_TOKEN or USER_NAME. Copy .env.example to .env and fill it in, or export the variables.')

# Personal access token (public read is enough): read:user + public_repo (classic),
# or fine-grained with Followers/Starring read + Metadata/Contents/Commit statuses read.
HEADERS = {'authorization': 'token ' + os.environ['ACCESS_TOKEN']}
USER_NAME = os.environ['USER_NAME']  # 'bcastelino'
BIRTHDAY = datetime.datetime(1997, 9, 14)
...
def format_plural(unit):
    """Returns 's' for a plural unit, otherwise ''."""
...
def graph_commits(start_date, end_date):
    """Uses GitHub's GraphQL v4 API to return total commit count in a range."""
...
def loc_counter_one_repo(owner, repo_name, data, cache_comment, history, addition_total, deletion_total, my_commits):
    """Recursively counts LOC of commits authored by me in one repository."""
...
def loc_query(owner_affiliation, comment_size=0, force_cache=False, cursor=None, edges=[]):
    """Queries all repositories (respecting owner_affiliation), 60 at a time, and builds the LOC cache."""
...
def stars_counter(data):
    """Counts total stars in repositories owned by me."""
    total_stars = 0
    for node in data:
...
def svg_overwrite(filename, age_data, commit_data, star_data, repo_data, contrib_data, follower_data, loc_data):
    """Parses an SVG file and updates elements with age, commits, stars, repos, and lines written."""
...
def justify_format(root, element_id, new_text, length=0):
    """Updates the element text and adjusts the sibling *_dots element to justify the value."""
...
def perf_counter(funct, *args):
    """Runs a function and returns its result and elapsed time."""
